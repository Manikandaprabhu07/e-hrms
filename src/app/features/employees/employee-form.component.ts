      ...
      <form [formGroup]="employeeForm" (ngSubmit)="onSubmit()">
          ...
          <app-card [title]="'System / Login Details'" [elevated]="true">
            <div class="form-grid">
              <div class="form-group required">
                ...
                <input type="text" formControlName="username" placeholder="employee.username" />
        ...
        <div class="form-actions">
          ...
          @if (currentStep() < steps.length - 1) {
            ...
            <button type="submit" class="btn btn-success" [disabled]="!employeeForm.valid">
              💾 Save Employee
            </button>
          }
        </div>
...
export class EmployeeFormComponent implements OnInit {
  ...
  onFileSelect(event: any, fieldName: string): void {
    ...
    if (file) {
      ...
      alert(`File "${file.name}" selected for ${fieldName}. (Mock - would upload to server)`);
    }
  }
    ...
    if (this.employeeForm.valid) {
      ...
      try {
        if (this.isEditMode() && id) {
          await this.employeeService.updateEmployee(id, formData);
